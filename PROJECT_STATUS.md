...
Updated: 2026-09-22. Scaffold version: **0.3.1** (root package.json is authoritative).
...
- `pnpm verify`: passed with formatting, lint, typecheck, 37 Node script tests, 33 workspace tests, frontend build, bundled API build, dependency-free production projection and repository policy.
- Clean generated project: application-only `AGENTS.md`, initialization guide notice, retained `docs/scaffold` reference and recipes, frozen install, full verification, dependency-free production projection, managed start/smoke, actual proxy CRUD, deliberately mismatched build rejection, graceful stop, restart identity change, owned child-exit cleanup, foreground readiness and cross-platform graceful shutdown passed.
